...
while len(dist) < 20:
    ...
    while True:
        ...
        f, t, Zxx = signal.stft(data, RATE, nperseg=NPERSEG, noverlap=0, boundary=None)
...
plt.show()
print(np.mean(dist))
# kmeans = KMeans(n_clusters=8).fit(np.array(dist).reshape(-1, 1)).cluster_centers_.reshape(-1)
# print(kmeans.min())
# print(kmeans)
